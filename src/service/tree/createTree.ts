...
export const createTreeData = (
  ...
  console.log('terminalItemArray :>> ', terminalItemArray);
  while (treeArray.length != 1) {
    treeArray = bunchItem(treeArray, feeRate, seed);
  }
  console.log("=================================");
  
  console.log('treeArray :>> ', treeArray[0].children);
  console.log("=================================");
  return treeArray[0];
};
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (bunchIterator == ONE_TIME_AIRDROP_SIZE) {
      ...
      newDataArray[currentPointer] = {
        ...newDataArray[currentPointer],
        utxo_value:
          sumAmount +  getRunestoneSize(childrenSize, seed) * feeRate,
      };
